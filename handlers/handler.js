...
module.exports = async (client) => {
    ...
    readdirSync("./Commands/Slash").forEach((dir) => {
      ...
      for (const cmd of commands) {
        ...
        if (command.name) {
          ...
          console.log(`${cmd} ยังไม่พร้อม`);
        }
      }
    });
    console.log(`${client.commands.size} โหลดคำสั่ง Slash แล้ว`);
  ...
  try {
    ...
    console.log(`${client.mcommands.size} โหลดคำสั่งข้อความแล้ว`);
  } catch (error) {
    console.log(error);
  }
  ...
  try {
    ...
    console.log(`${eventCount} โหลดกิจกรรมแล้ว`);
  } catch (e) {
    console.log(e);
  }
};
