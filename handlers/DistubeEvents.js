        ...
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setAuthor({
                name: `เพิ่มเพลย์ลิสต์ลงในคิวแล้ว`,
          ...
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                ...
                } ตัดการเชื่อมต่อจาก <#${voiceMap.get(
        ...
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setTitle(
                `ไม่พบเพลงที่เกี่ยวข้องสำหรับ \`${queue?.songs[0].name}\``
              ),
          ],
        })
        .then((msg) => {
      ...
      queue.textChannel
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(`หมดคิวแล้ว! ไม่มีเพลงให้เล่นอีกต่อไป`),
          ],
        })
        .then((msg) => {
          setTimeout(() => {
        ...
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(
                `${client.config.emoji.ERROR} ไม่พบผลลัพธ์สำหรับ \`${quary}\`!`
              ),
          ],
        })
        .then((msg) => {
        ...
        queue.textChannel
          .send({
            embeds: [
              new EmbedBuilder()
                .setColor(client.config.embed.color)
                .setDescription("ช่องว่างเปล่า ออกจากช่อง"),
            ],
          })
          .then((msg) => {
            setTimeout(() => {
          ...
          case "autoplay":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} คุณต้องเข้าร่วม __My__ Voice Channel `
                );
              } else if (!queue) {
                return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} ฉันไม่ได้เล่นตอนนี้ `
                );
              } else if (checkDJ) {
                return send(
                  interaction,
                  `${client.config.emoji.SUCCESS} คุณไม่ใช่ดีเจและคุณไม่ใช่ผู้ขอเพลง..`
                );
              } else if (!queue.autoplay) {
                queue.toggleAutoplay();
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} เปิดใช้งานการเล่นอัตโนมัติ !! `
                );
              } else {
                queue.toggleAutoplay();
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} การเล่นอัตโนมัติถูกปิดใช้งาน !!.`
                );
              }
            }
            break;
          case "skip":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  `${client.config.emoji.SUCCESS}  เพลงข้าม !!`
                );
              }
            }
            break;
          case "stop":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} เพลงหยุดและช่องซ้าย !!.`
                );
              }
            }
            break;
          case "pauseresume":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} คิวดำเนินการต่อ!! `
                );
              } else if (!queue.paused) {
                await queue.pause();
                return send(
          ...
          case "loop":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  `${client.config.emoji.SUCCESS} เปิดเพลงวนไป !!`
                );
              } else if (queue.repeatMode === 1) {
                await queue.setRepeatMode(2);
                return send(
                  interaction,
                  `${client.config.emoji.SUCCESS} เปิดคิววนซ้ำ !!`
                );
              } else if (queue.repeatMode === 2) {
                await queue.setRepeatMode(0);
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} ปิดลูป !!`
                );
              }
            }
            break;
      ...
      if (message.channelId === musicchannel.id) {
        ...
        if (message.author.bot) {
          ...
          if (
            ...
            return client.embed(
              message,
              `** ${client.config.emoji.ERROR} ฉันไม่ได้รับอนุญาตให้เข้าร่วมช่องเสียง **`
            );
