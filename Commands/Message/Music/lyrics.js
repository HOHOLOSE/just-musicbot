...
module.exports = {
  name: "lyrics",
  aliases: ["lr"],
  description: `ค้นหาเนื้อเพลงของเพลงปัจจุบัน`,
  ...
  run: async (client, message, args, prefix, queue) => {
    ...
    if (!lyrics)
      return client.embed(message, `ไม่พบเนื้อเพลงสำหรับ \`${songname}\``);

    let embeds = string.map((str) => {
      return new EmbedBuilder()
        .setColor(client.config.embed.color)
        .setAuthor({ name: `เนื้อเพลงของ ${songname}`, iconURL: song.thumbnail })
        .setDescription(`${str || `ไม่พบเนื้อเพลงสำหรับ \`${songname}\``}`)
