...
client.on("messageCreate", async (message) => {
      ...
      return message.reply({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(
              ` ${emoji.SUCCESS} หากต้องการดูประเภทคำสั่งทั้งหมดของฉัน  \`/help\` หรือ \`${prefix}help\``
            ),
        ],
      });
    }
  ...
  if (command) {
    ...
    if (
      ...
      return client.embed(
        message,
        `คุณไม่ได้รับอนุญาตให้ใช้ \`${command.name}\` คำสั่ง!!`
      );
    } else if (
      ...
      return client.embed(
        message,
        `ฉันไม่ได้รับอนุญาตให้เรียกใช้ \`${command.name}\` คำสั่ง!!`
      );
    } else if (cooldown(message, command)) {
      return client.embed(
        message,
        ` คุณอยู่ในคูลดาวน์ , รอ \`${cooldown(
          message,
          command
        ).toFixed()}\` วินาที`
      );
      ...
      return client.embed(message, `${emoji.ERROR} เพลงไม่เล่น`);
